...
function processV1Request (request, response) {
  ...
  const actionHandlers = {
    ...
    // Topics Intent
    'Topics': () => {
      var topic = request.body.result.parameters.Topics;
      var tech = request.body.result.parameters.technology;
	  var number = request.body.result.parameters.number;
	  if ( number != null && tech == 'analytics' ) {
		  sendResponse('Analytics has overall *465* applications, are you looking for some particluar application?');
	  }
      var appcenter = 'https://www.sapappcenter.com/apps/10611#!overview';
	  var details = 'https://www.infosys.com/SAP/offerings/Documents/SAP-analytics-custom-extensions.pdf';
	  
      // Use the Actions on Google lib to respond to Google requests; for other requests use JSON
      if (userip_tech === 'analytics') {
        sendResponse('The appcenter link that you have requested is: ' + appcenter);
      } else {
        sendResponse('The appcenter link that you have requested is: ' + appcenter + '		  More details can be found in ' + details ); // Send simple response to user
      }
    },
